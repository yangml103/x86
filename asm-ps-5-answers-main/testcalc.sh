...
#         CALC_DATA_END
#         The program should write this two standard out.  We save
#         this output to a file resbin file

if [[ -a $binary.resbin ]]; then rm $binary.resbin; fi

echo "running $binary saving standard output to $binary.resbin"

./$binary > $binary.resbin
...
if [[ -n $reference ]]; then
    refchanges=$(diff <(xxd -g1 data/reference_$binary.prebin ) <(xxd -g1 data/reference_$binary.resbin))
